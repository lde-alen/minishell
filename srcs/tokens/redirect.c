...
/*   Updated: 2022/09/29 06:51:10 by asanthos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_redir_type(t_lex *lex)
{
	if (lex->cmd->redir->left_r > lex->cmd->redir->left_dr)
		redirect_in(lex, ft_strdup(lex->cmd->redir->file[lex->cmd->redir->left_r]));
	...
	if (lex->cmd->redir->right_r > lex->cmd->redir->right_dr)
		redirect_out(lex, ft_strdup(lex->cmd->redir->file[lex->cmd->redir->right_r]));
	else if (lex->cmd->redir->right_dr > lex->cmd->redir->right_r)
		append_out(lex, ft_strdup(lex->cmd->redir->file[lex->cmd->redir->right_dr]));
	else
